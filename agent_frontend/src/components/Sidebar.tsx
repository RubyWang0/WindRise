...
export const Sidebar: React.FC = () => {
  ...
  return (
    <div className={cn(
      ...
      {/* Slideout Sidebar Content Container */}
      <div className={cn(
        "absolute inset-y-0 left-0 w-64 border-r border-zinc-300 dark:border-zinc-800 bg-[#EDF0F2] dark:bg-[#030303] flex flex-col h-full text-zinc-800 dark:text-zinc-100 transition-all duration-300 ease-in-out z-40 overflow-hidden",
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3 overflow-hidden flex-1">
                  ...
                  <span className={cn(
                    "text-sm truncate",
                    isActive ? "text-zinc-900 dark:text-white font-medium" : "text-zinc-500 dark:text-zinc-455 group-hover:text-zinc-800 dark:group-hover:text-zinc-200"
                  )}>
                    {firstMsg}
                  </span>
                </div>
        ...
        <div className="p-4 border-t border-zinc-300 dark:border-zinc-800 bg-[#EDF0F2] dark:bg-[#030303] transition-colors duration-200 flex-shrink-0">
          <div className="flex items-center space-x-3 px-2 py-2 rounded-lg hover:bg-white/80 dark:hover:bg-zinc-900 transition-colors cursor-pointer text-zinc-700 dark:text-zinc-300">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-[10px] text-zinc-455 dark:text-zinc-500 truncate">Free Plan</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className={cn(
          "absolute top-1/2 transform -translate-y-1/2 z-50 flex items-center justify-center cursor-pointer transition-all duration-300",
          "w-5 h-16 rounded-r-xl border border-l-0 bg-[#F0F2F4]/90 dark:bg-[#101010]/90 backdrop-blur-md",
          "border-zinc-300 dark:border-zinc-700 hover:border-zinc-400 dark:hover:border-zinc-500",
          "text-zinc-400 hover:text-zinc-600 dark:text-zinc-600 dark:hover:text-zinc-300",
          "hover:shadow-[2px_0_8px_rgba(255,255,255,0.08)] dark:hover:shadow-[2px_0_12px_rgba(255,255,255,0.1)]",
          isCollapsed ? "left-0" : "left-64"
        )}
      >
        {/* Indicator line inside button */}
        <div className={cn(
          "absolute left-0 top-1/4 bottom-1/4 w-[2px] transition-all duration-300",
          isCollapsed ? "bg-zinc-400 dark:bg-zinc-700" : "bg-zinc-500 dark:bg-zinc-400 shadow-[0_0_6px_rgba(255,255,255,0.4)]"
        )} />
        
        {isCollapsed ? <ChevronRight size={12} className="ml-0.5" /> : <ChevronLeft size={12} className="-ml-0.5" />}
      </button>
    </div>
  );
};
